...
const NewReleased = ({ count, unique = false }) => {
    ...
    return (
        <div className='w-full min-h-[38vh] text-light' id='new_Release'>
            ...
            <div className='w-full flex md:flex-row flex-col justify-center md:flex-wrap md:gap-0 gap-16 md:justify-between  px-14 relative'>
                {
                    allSortedSongs.length > 0 &&
                    allSortedSongs.map((song, index) => (
                        <>
                            <figure className=' w-[225px] md:w-[150px] relative  cursor-pointer' key={index}>
                                <div className='group relative w-[225px]  h-[225px] md:w-[150px] md:h-[150px]'>
                                    ...
                                    }
                                    {
                                        ...
                                        <div className='absolute  bottom-0 w-full h-full bg-gradient-to-t from-white to-transparent flex justify-center items-end pb-3 rounded-full cursor-pointer z-20 '
                                            onClick={() => handleAudio(index)}
                                        >
                                ...
                                <figcaption className='text-center  mt-2 text-3xl md:text-[1rem] font-extrabold tracking-wide'>{song?.songName}</figcaption>
                            </figure>
