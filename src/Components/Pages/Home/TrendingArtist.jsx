...
const TrendingArtist = () => {
  return (
    <div className="w-full min-h-[38vh] text-light  " id="trending_Artist">
      <p className=" text-4xl font-serif mb-6 pl-6  font-extrabold">
        Trending Artist's - <sup>Coming Soon</sup>{" "}
      </p>
      <div className="w-full px-14  ">
        <Slider {...settings}>
          {TrendingArtistData.map((artist) => {
            return (
              <NavLink state={{ artist }} key={artist?.imgSrc}>
                <figure className="bg-secondary pt-4 px-4 pb-4 rounded-xl w-[250px]  shadow-[inset_5px_-5px_5px_#101215,inset_-5px_5px_5px_#424655]">
                  <img
                    src={artist?.imgSrc}
                    className=" w-[200px] rounded-full h-[200px] m-auto object-cover"
                  />
                  <figcaption className="text-center mt-2 text-[1rem] font-extrabold tracking-wide">
                    {artist?.name}
                  </figcaption>
                </figure>
              </NavLink>
