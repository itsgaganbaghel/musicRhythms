...
const AlbumSideBar = () => {
  ...
  let linksArray = [
    ["Home", "#Hero"],
    ["Top Albums", "#top_Album"],
    ["Trending Artist", "#trending_Artist"],
    [" New Release", "#new_Release"],
    ["Random Songs", "#random_Songs"],
  ];
          ...
          {isVisibility == true && targetModel == "logout" && (
            <Modal>
              <div className="px-44 rounded-2xl py-24 shadow-[inset_5px_-5px_5px_#101215,inset_-5px_5px_5px_#424655] bg-secondary ">
                <p className="text-3xl font-serif text-light pb-10">
                  Are You want to Log Out ?
                </p>
                <div className="flex gap-10 ">
                  <button
                    className="w-full border   text-accent text-center py-2 px-4 rounded-lg hover:bg-light hover:text-primary transition-all duration-300"
                  ...
                  <button
                    className="w-full border  text-accent  text-center py-2 px-4 rounded-lg hover:bg-light hover:text-primary transition-all duration-300"
                    onClick={() => {
                      setTargetModel(null), setVisibility(false);
                    }}
                  >
                    No
                  </button>
